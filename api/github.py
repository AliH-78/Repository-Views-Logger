...
class GitHubRequestSession(requests.Session):
    ...
    def request(self, *args, **kwargs):
        ...
        if request.status_code // 100 != 2:
            raise exceptions.GitHubRequestError(request.status_code, f"[{request.status_code}] {request.json()['message']}")
...
def handle_popular_files_information(traffic_information):
    return {i["path"] : {"view_count" : i["count"],
                         "unique_views" : i["uniques"]} for i in traffic_information}
...
class GitHubAccount:
    ...
    def get_view_count(self, repository):
        ...
        return scrape_traffic_view_request.json()
